...
class ImportsLoggerSpec extends UnitSpec with MockitoSugar {
  ...
  implicit val validatedRequest = ValidatedRequest[AnyContent](requestData, requestMock)
  when(validatedRequest.requestData.conversationId).thenReturn("conversation-id")
  when(validatedRequest.requestData.clientId).thenReturn("some-client-id")
  when(validatedRequest.requestData.requestedApiVersion).thenReturn("1.0")
